      ...
      <div className="hidden print:block print-container print-a4 bg-white text-black absolute inset-0 z-50 p-8 min-h-screen text-left" style={{ backgroundColor: '#ffffff', color: '#000000' }}>
        ...
        {cleanNotes && (
          <div className="mt-8 border-t pt-4 text-left">
            <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">Notes / Requests:</p>
      ...
      <div className="bg-dark-800 border border-dark-700 overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-dark-900 border-b border-dark-700">
            <tr>
              <th onClick={() => handleSort('created_at')} className="p-4 font-medium text-gray-400 cursor-pointer hover:text-gold-500 select-none transition-colors">
                ID / Booked{renderSortIndicator('created_at')}
              </th>
              <th className="p-4 font-medium text-gray-400 select-none">Guest</th>
              <th className="p-4 font-medium text-gray-400 select-none">Room</th>
              <th onClick={() => handleSort('date')} className="p-4 font-medium text-gray-400 cursor-pointer hover:text-gold-500 select-none transition-colors">
                Dates{renderSortIndicator('date')}
              </th>
              <th onClick={() => handleSort('amount')} className="p-4 font-medium text-gray-400 cursor-pointer hover:text-gold-500 select-none transition-colors">
                Amount{renderSortIndicator('amount')}
              </th>
              <th className="p-4 font-medium text-gray-400 select-none">Discount</th>
              <th onClick={() => handleSort('status')} className="p-4 font-medium text-gray-400 cursor-pointer hover:text-gold-500 select-none transition-colors">
                Status{renderSortIndicator('status')}
              </th>
              <th className="p-4 font-medium text-gray-400 text-right select-none">Actions</th>
            </tr>
          </thead>
          <tbody className="text-sm">
            {loading ? (
              <tr>
                <td colSpan="8" className="p-8 text-center text-gray-400"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-brand-500 mx-auto"></div></td>
              </tr>
            ) : reservations.length === 0 ? (
              <tr>
                <td colSpan="8" className="p-8 text-center text-gray-400">No reservations found.</td>
              </tr>
            ) : (
                  ...
                  <td className="p-4">
                    {res.status === 'confirmed' && res.payment_status !== 'paid' ? (
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs capitalize ${
                        ...
                        res.status === 'no_show' ? 'bg-gray-500/10 text-gray-400' :
                        'bg-gray-500/10 text-gray-400'
                      }`}>
                        {res.status.replace('_', ' ')}
                      </span>
                    )}
                    <span className="block text-xs text-gray-500 mt-2">{res.booking_source?.replace('_', ' ')}</span>
                  </td>
          ...
          <div className="bg-dark-800 border border-dark-700 p-6 w-full max-w-2xl relative animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto custom-scrollbar print:hidden">
            ...
            <div className="print:hidden">
              <div className="absolute top-4 right-4 flex gap-2">
                <button onClick={() => setViewBooking(null)} className="text-gray-400 hover:text-white bg-dark-700 p-1.5 rounded"><X size={20}/></button>
              </div>
              
              <div className="flex justify-between items-start mb-6 border-b border-dark-700 pb-4">
                <div>
                  <h2 className="text-xl font-semibold text-white">Booking Details</h2>
                  <p className="text-gray-400 text-sm">Ref: {viewBooking.booking_reference}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div className="space-y-1">
                  <p className="text-xs text-gray-500 uppercase font-bold">Guest Info</p>
                  ...
                  {viewBooking.guest_email && <p className="text-sm text-gray-400">{viewBooking.guest_email}</p>}
                  {viewBooking.guest_phone && <p className="text-sm text-gray-400">{viewBooking.guest_phone}</p>}
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-gray-500 uppercase font-bold">Stay Info</p>
                  <p className="text-white">{viewBooking.rooms?.name}</p>
                  <p className="text-sm text-gray-400 flex items-center gap-1"><Calendar size={14}/> {format(new Date(viewBooking.check_in_date), 'MMM dd')} - {format(new Date(viewBooking.check_out_date), 'MMM dd, yyyy')}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {servicesLoading ? (
                  <p className="text-gray-500 text-sm italic">Loading services...</p>
                ) : bookingServices.length === 0 ? (
                  <p className="text-gray-500 text-sm italic bg-dark-900 p-4 rounded border border-dark-700">No additional services booked.</p>
                ) : (
