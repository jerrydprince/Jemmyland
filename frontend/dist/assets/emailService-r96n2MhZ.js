import{s as S}from"./index-DCylpLB6.js";const v=async({to:t,subject:e,html:r,from:o})=>{try{console.log(`[Resend Client] Dispatching email to: ${t} via backend proxy...`);const a=await fetch("/api/email/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:t,subject:e,html:r,from:o})});if(a.ok)return{success:!0,id:(await a.json()).id||"msg_"+Math.random().toString(36).substr(2,9)};const p=await a.text();console.warn(`[Resend Client] Backend endpoint failed (${a.status}): ${p}. Falling back...`)}catch(i){console.warn(`[Resend Client] Backend proxy unreachable: ${i.message}. Falling back...`)}return console.warn(`[Resend Client] Simulating email delivery to: ${t}`),await new Promise(i=>setTimeout(i,800)),{success:!0,simulated:!0}},Z=async(t,e)=>{if(!e)return console.warn(`[Automation Engine] Trigger aborted for ${t}: Missing payload.`),{success:!1,reason:"Missing booking payload"};try{console.log(`[Automation Engine] Triggered event: "${t}"`);const{data:r}=await S.from("system_settings").select("setting_key, setting_value").in("setting_key",["notification_engine_active","contact_logo","contact_address","contact_phone","contact_email","system_theme"]),o=r?.reduce((d,s)=>(d[s.setting_key]=s.setting_value,d),{})||{};if(!(o.notification_engine_active==="true"||o.notification_engine_active===!0||o.notification_engine_active===void 0))return console.log("[Automation Engine] Engine is toggled offline in System Control."),{success:!1,reason:"Notification engine inactive"};const a=o.system_theme||"theme-luxe-gold",h={"theme-slate-dark":"#64748B","theme-luxe-gold":"#DF6853","theme-emerald-green":"#10B981","theme-royal-blue":"#3B82F6","theme-sunset-orange":"#F97316","theme-rose-burgundy":"#F43F5E","theme-midnight-purple":"#A855F7","theme-ocean-teal":"#14B8A6"}[a]||"#DF6853";let c=o.contact_logo||"https://images.unsplash.com/photo-1512918728675-ed5a9ecdebfd?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80";c&&c.startsWith("/")&&(c=window.location.origin+c);const F=o.contact_address||"Plot 572 Iduwa Ogenyi Street Mabushi, Off Ahmadu Bello Way, Abuja",j=o.contact_phone||"08033214684, 08062332639, 08171278657",T=o.contact_email||"info@jemmylandhotels.com",{data:u,error:y}=await S.from("automation_rules").select("*, notification_templates(*)").eq("trigger_event",t).eq("is_active",!0);if(y)return console.error("[Automation Engine] Failed to fetch active rules:",y),{success:!1,error:y.message};if(!u||u.length===0)return console.log(`[Automation Engine] Zero active rules configured for event "${t}".`),{success:!0,count:0};console.log(`[Automation Engine] Processing ${u.length} automations for "${t}"...`);const A=[];for(const d of u){const s=d.notification_templates;if(!s)continue;const g=e.profiles||{},$=e.guest_name||`${g.first_name||""} ${g.last_name||""}`.trim()||"Valued Guest",z=e.guest_email||e.email||g.email||"guest@example.com",B=e.guest_phone||e.phone||g.phone||"N/A",P=e.booking_reference||e.id||"BKG-MOCK",H=e.check_in_date||e.check_in||"N/A",L=e.check_out_date||e.check_out||"N/A",l=s.channel==="email"?z:B;if(!l||l==="N/A"){console.warn(`[Automation Engine] Skipping rule "${d.name}": No recipient detail.`);continue}const O=e.room_number||e.rooms&&e.rooms.room_number||"N/A",k=e.room_details||e.rooms&&e.rooms.name||"Premium Suite",E=e.total_amount||e.total_amount_ngn||e.total_price||"0.00",N=e.total_paid||e.amount_paid||e.amount_paid_ngn||"0.00",I=e.balance_due!==void 0?e.balance_due:(Number(E)-Number(N)).toFixed(2),J=e.payment_status||"Pending",R=e.payment_amount||e.amount||"0.00",G=e.payment_ref||e.payment_reference||"N/A",W=e.payment_method||"N/A",q=e.payment_date||new Date().toLocaleDateString(),U=e.invoice_number||"INV-"+P,C=n=>n?n.replace(/{{guest_name}}/g,$).replace(/{{booking_ref}}/g,P).replace(/{{check_in}}/g,H).replace(/{{check_out}}/g,L).replace(/{{room_number}}/g,O).replace(/{{room_details}}/g,k).replace(/{{total_amount}}/g,Number(E).toLocaleString()).replace(/{{total_paid}}/g,Number(N).toLocaleString()).replace(/{{balance_due}}/g,Number(I).toLocaleString()).replace(/{{payment_status}}/g,J).replace(/{{payment_amount}}/g,Number(R).toLocaleString()).replace(/{{payment_ref}}/g,G).replace(/{{payment_method}}/g,W.toUpperCase()).replace(/{{payment_date}}/g,q).replace(/{{invoice_number}}/g,U):"",M=C(s.subject||"Jemmyland Hotels Update"),x=C(s.body||"");let m="failed",b=null,f=!1;if(s.channel==="email"){const n=`
          <div style="font-family: 'Outfit', sans-serif; padding: 30px; color: #1f2937; max-width: 600px; margin: auto; border: 1px solid #e5e7eb; border-top: 6px solid ${h}; border-radius: 16px; background-color: #ffffff;">
            <div style="text-align: center; border-bottom: 1px solid #f3f4f6; padding-bottom: 20px; margin-bottom: 20px;">
              ${c?`<img src="${c}" alt="Jemmyland Hotels" style="max-height: 50px; object-fit: contain; margin-bottom: 8px; border-radius: 4px;" />`:""}
            ...
            <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #f3f4f6; text-align: center; font-size: 12px; color: #9ca3af;">
              ...
              <p style="margin: 5px 0 0 0;">Phones: ${j} | Email: ${T}</p>
            </div>
          </div>
        `;if(l!=="booking@jemmylandhotels.com")try{console.log("[Automation Engine] Forwarding admin copy of booking update to booking@jemmylandhotels.com...");const w=`
